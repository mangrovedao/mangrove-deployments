...
function createFilterFunction(criteria: DeploymentFilter) {
  return (deployment: VersionDeployments) => {
    ...
    if (criteriaWithDefaults.deploymentName && deployment.deploymentName != criteriaWithDefaults.deploymentName) return false;

    return true;
  }
}

export const findDeployment = (criteria: DeploymentFilter = DEFAULT_FILTER, deployments: VersionDeployments[]): VersionDeployments | undefined => {
  return deployments.find(createFilterFunction(criteria));
}

export const findAllDeployments = (criteria: DeploymentFilter = DEFAULT_FILTER, deployments: VersionDeployments[]): VersionDeployments[] => {
